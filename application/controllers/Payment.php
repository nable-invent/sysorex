...
class Payment extends CI_Controller{
    public function __construct(){
        parent::__construct();
        echo "constructor";
        $this->load->model("ResMsgDTO");
        $this->load->model("VtransactSecurity");
        $this->load->model("HTTPPost");
        $this->load->model("ReqMsgDTO");
    }
    public function create(){
        

	//create an object of Request Message
	$reqMsgDTO = new stdClass();
    $reqMsgDTO->mid ="WL0000000027698"; 
    $reqMsgDTO->orderId = "1625653555842620";
    $reqMsgDTO->trnAmt="100";
    $reqMsgDTO->trnCurrency="INR";
    $reqMsgDTO->trnRemarks="";
    $reqMsgDTO->meTransReqType="S";
    // $reqMsgDTO->recurrPeriod
    // $reqMsgDTO->recurrDay
    // $reqMsgDTO->noOfRecurring
    $reqMsgDTO->responseUrl="https://sysorexworld.com";
    
    $reqMsgDTO->reqMsg="";
    // $reqMsgDTO->statusDesc
    $reqMsgDTO->enckey = "6375b97b954b37f9569".rand(1000,9999)."7e5753ee6";


	$merchantRequest = "";
	
	$msg = $this->generateTrnReqMsg($reqMsgDTO);
	
	
		$merchantRequest = $msg;
		$dataCollection = json_encode($msg);
// 		echo $dataCollection;
// 		$url = 'https://cgt.in.worldline.com/ipg/doMEPayRequest';
		$dataArr['mReq'] = $msg->reqMsg;
		$dataArr['mid']="WL0000000027698"; 
            $this->load->view("pay",$dataArr);
        }
    //payment
    function generateTrnReqMsg($reqMsgDTO)
        { 
            // print_r($reqMsgDTO); die;
		 
            try
            {
			
         
		    $merchantReqStr= $reqMsgDTO->mid 	."|". $reqMsgDTO->orderId  ."|". $reqMsgDTO->trnAmt ."|".$reqMsgDTO->trnCurrency     ."|". $reqMsgDTO->trnRemarks  ."|". $reqMsgDTO->meTransReqType 
			."||||".$reqMsgDTO->responseUrl."||||||||||NANA";
			
		
	        $enc_data = $this->encryptValue($merchantReqStr,$reqMsgDTO->enckey); 			
			
           	 	$reqMsgDTO->reqMsg =$enc_data;
				$reqMsgDTO->statusDesc = "Success";
